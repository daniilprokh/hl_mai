...
class AutorizationHandler : public Poco::Net::HTTPRequestHandler {
 ...
 private:
  ...
  void HandleUserRequest(const Poco::Net::HTMLForm &form,
    ...
    uint64_t id = stoull(form.get(properties[0]));

    bool cached;
    std::istringstream(form.get(properties[1])) >> std::boolalpha >> cached;
    if (cached) {
      auto user = database::UserConverter::GetInstance().UserFromCache(id);
      if (user.has_value()) {
        std::cout << "user load from cache" << std::endl;
  ...
  void HandleSaveRequest(const Poco::Net::HTMLForm &form,
    ...
    if (CheckHtmlForm(form, properties, bad_request_message)) {
      ...
      if (message.empty())
      {
        ...
        bool cached;
        std::cout << form.get(properties.back()) << std::endl;
        std::istringstream(form.get(properties.back())) >> std::boolalpha >> cached;
        if (cached) {
